...
def load_npz(path: Path) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    if not path.exists():
...
def build_loader(path: Path, batch_size: int, shuffle: bool) -> DataLoader[tuple[torch.Tensor, ...]]:
    input_ids, attention_mask, labels = load_npz(path)
...
def compute_metrics(y_true_idx: np.ndarray, y_pred_idx: np.ndarray, labels: list[str]) -> dict[str, Any]:
...
def load_train_cfg(config_dir: str) -> tuple[LSTMTrainConfig, list[str], int]:
    ...
    if not preprocessing_meta_path.exists():
        raise ValueError("Run scripts/prepare_lstm_data.py first (metadata missing).")
    if not vocab_path.exists():
        raise ValueError("Run scripts/prepare_lstm_data.py first (vocab missing).")
...
def run_category_lstm_training(config_dir: str = "configs", epochs: int | None = None) -> dict[str, Any]:
    train_cfg, labels, pad_idx = load_train_cfg(config_dir=config_dir)
    if epochs is not None:
        if epochs < 1:
            raise ValueError("--epochs must be >= 1")
        train_cfg.epochs = epochs
    set_torch_seed(train_cfg.seed)

    device = torch.device(train_cfg.device)
    train_loader = build_loader(Path("artifacts/lstm_preprocessing/train.npz"), train_cfg.batch_size, True)
    val_loader = build_loader(Path("artifacts/lstm_preprocessing/val.npz"), train_cfg.batch_size, False)
    test_loader = build_loader(Path("artifacts/lstm_preprocessing/test.npz"), train_cfg.batch_size, False)
    ...
    val_metrics = compute_metrics(val_true, val_pred, labels=labels)
    test_metrics = compute_metrics(test_true, test_pred, labels=labels)
    ...
    print("LSTM category training completed.")
